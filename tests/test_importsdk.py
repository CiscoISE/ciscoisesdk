...
class TestImportSDK:
    ...
    def setup(self, base_url):
        ...
        self.verify = False
        self.debug = True
        self.version = "3.3_patch_1"


@pytest.fixture
def import_fixture(mock_identity_services_engine_server, base_url):
...
def reset_env_vars(monkeypatch):
    monkeypatch.delenv("IDENTITY_SERVICES_ENGINE_BASE_URL", raising=False)
    monkeypatch.delenv("IDENTITY_SERVICES_ENGINE_USERNAME", raising=False)
    monkeypatch.delenv("IDENTITY_SERVICES_ENGINE_PASSWORD", raising=False)
    monkeypatch.delenv("IDENTITY_SERVICES_ENGINE_ENCODED_AUTH", raising=False)
...
def test_get_enviroment_before_import_1(monkeypatch, reset_env_vars, import_fixture):
    ...
    monkeypatch.setenv("IDENTITY_SERVICES_ENGINE_USERNAME", import_fixture.username)
    monkeypatch.setenv("IDENTITY_SERVICES_ENGINE_PASSWORD", import_fixture.password)
    monkeypatch.setenv("IDENTITY_SERVICES_ENGINE_BASE_URL", import_fixture.base_url)
    from ciscoisesdk import IdentityServicesEngineAPI
    IdentityServicesEngineAPI(verify=import_fixture.verify, debug=import_fixture.debug)
    return True
...
def test_get_enviroment_before_import_2(monkeypatch, reset_env_vars, import_fixture):
    """Tests if the package gets the values of the env variables before import."""
    monkeypatch.setenv("IDENTITY_SERVICES_ENGINE_ENCODED_AUTH", import_fixture.encoded_auth)
    monkeypatch.setenv("IDENTITY_SERVICES_ENGINE_BASE_URL", import_fixture.base_url)
    from ciscoisesdk import IdentityServicesEngineAPI
    IdentityServicesEngineAPI(verify=import_fixture.verify, debug=import_fixture.debug)
    return True
...
def test_get_enviroment_exception_1(monkeypatch, reset_env_vars, import_fixture):
    ...
    monkeypatch.setenv("IDENTITY_SERVICES_ENGINE_BASE_URL", import_fixture.base_url)
    from ciscoisesdk import IdentityServicesEngineAPI, AccessTokenError
